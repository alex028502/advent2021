...
(define (all-directions)
  (filter (lambda (point) (> (apply + (map abs point)) 0))
...
(define (char->number char)
  (/> char list list->string string->number))

(define (hash-board rows)
  (/> rows
      (curry map hash-row (range (length rows)))
      (curry apply append)
      make-immutable-hash))
...
(define (hash-row y row)
  (map cons (map (curryr list y) (range (length row))) row))
...
(define (take-turns turns board)
  (foldl (lambda (_ situation) (apply take-turn situation))
         (list board 0)
         (range turns)))

(define (take-turn board score)
  (/> board increment-all-points resolve-flashes (curry tally-and-reset score)))
...
(define (resolve-flashes board)
  (let ([new-board (do-one-flash board)])
    (if new-board (resolve-flashes new-board) board)))
...
(define (increment-neighbours location board)
  (increment-points board (all-neighbouring-coordinates location)))
...
(define (increment-points board points)
  (foldl (lambda (point board)
           (if (hash-ref board point #f) (hash-update board point add1) board))
         board
         points))
